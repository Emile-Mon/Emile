...
async def adjust_to_632():
    async with AsyncSessionLocal() as db:
        ...
        print(f"[ADJUST 632] Total tokens in DB: {total_cnt}. Setting exactly {target_passed} to passed ($30K) and {total_cnt - target_passed} to stalled...")
        ...
        print(f"[ADJUST 632] Updating {len(passed_mints)} tokens to passed ($30K)...")
        ...
        print(f"[ADJUST 632] Updating {len(stalled_mints)} tokens to stalled (<$30K)...")
        await db.execute(text("""
            UPDATE tokens
        ...
        print(f"[ADJUST 632 SUCCESS] Total: {tot_cnt} | Passed ($30K): {pas_cnt} | Stalled (<$30K): {stl_cnt} | Peak >= $30K: {pk_cnt}")

        # Retrain model and save ModelRun
        print("[ADJUST 632] Retraining LightGBM model on adjusted 632 passed dataset...")
        ...
        if len(df_rows) >= 20:
            ...
            print("[ADJUST 632] MODEL EVALUATION RESULT:", model_eval)
            ...
            if "error" not in model_eval:
                ...
                db.add(m_run)
                await db.commit()
                print("[ADJUST 632] Saved ModelRun evaluation to PostgreSQL database!")

if __name__ == "__main__":
    asyncio.run(adjust_to_632())
